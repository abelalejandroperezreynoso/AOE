...
function timbering(ctx, a, b, h, k0, wood) {
  ...
  const bays = Math.max(1, Math.round(len / 22));
...
function slopeTexture(ctx, rA, rB, eA, eB, tone, mat) {
  ...
  if (mat === 'shingle') {
    ...
    const rows = clamp(Math.round(len / 3.6), 5, 16);
    const cols = clamp(Math.round(width / 4.2), 5, 32);
    const at = (s, t) => lerp2(lerp2(rA, rB, s), lerp2(eA, eB, s), t);
    const shades = [0.16, -0.13, 0.05, -0.2, 0.1, -0.04, -0.09];
...
/**
 * Dientes de ripia asomando por el alero. Una tablilla nunca queda a ras de
 * la de al lado, y ese borde mordido es lo que distingue un tejado de madera
 * de uno de teja incluso de lejos.
 */
function shingleEave(ctx, eA, eB, tone) {
  const n = Math.max(4, Math.round(Math.hypot(eB[0] - eA[0], eB[1] - eA[1]) / 4.4));
  const drop = [2.6, 3.8, 1.9, 3.2, 2.3, 4, 2.8];
  for (let i = 0; i < n; i++) {
    const p = lerp2(eA, eB, i / n), q = lerp2(eA, eB, (i + 1) / n);
    const d = drop[i % drop.length];
    poly(ctx, [p, q, [q[0], q[1] + d], [p[0], p[1] + d]],
      i % 2 ? shade(tone, -0.2) : shade(tone, -0.1));
  }
}
...
function roofOn(ctx, x, y, w, d, base, rh, c1, c2, c3, mat, o = {}) {
  ...
  const ov = thatch ? 0.13 : mat === 'shingle' ? 0.16 : 0.1; // vuelo del alero
...
function shedRoof(ctx, x, y, w, d, hi, lo, dir, slope, edge, mat) {
  ...
  if (mat === 'shingle') shingleEave(ctx, eA, eB, slope);
  else poly(ctx, [eA, eB, [eB[0], eB[1] + th], [eA[0], eA[1] + th]], edge);
  if (thatch) {
...
/**
 * Entramado del hastial. El centro urbano lo lleva 'lucido', con su colgadizo
 * y el ventanuco del desván; la casa de aldea, sólo el tirante, el pendolón y
 * dos tornapuntas, que es lo que se ve en cualquier casa.
 */
function gableTimbers(ctx, a, b, apex, wood, woodD, style = 'plain') {
  ...
  if (style === 'rich') {
    ...
    beam(lerp2(a, b, 0.16), lerp2(mid, apex, 0.72), 1.8, woodD);
...
/**
 * Cesto de mimbre lastrado: la peana que sujeta el poste de la grúa del
 * campamento maderero. Va tejido, de ahí las costillas y los aros.
 */
function wickerBase(ctx, cx, cy, r, h, wL, wM, wD) {
  const top = r * 0.34;
  poly(ctx, [[cx - r, cy], [cx - top, cy - h], [cx + top, cy - h], [cx + r, cy]], wM);
  ctx.fillStyle = wM;
  ctx.beginPath(); ctx.ellipse(cx, cy, r, r * 0.4, 0, 0, Math.PI); ctx.fill();
  // La mitad de sotavento, en sombra.
  poly(ctx, [[cx + top * 0.2, cy - h], [cx + top, cy - h], [cx + r, cy],
    [cx + r * 0.3, cy + r * 0.36]], shade(wM, -0.14));
  ctx.fillStyle = wL;
  ctx.beginPath(); ctx.ellipse(cx, cy - h, top, top * 0.44, 0, 0, Math.PI * 2); ctx.fill();
  // Costillas del tejido, de la boca al asiento.
  ctx.strokeStyle = wD; ctx.lineWidth = 0.8; ctx.lineCap = 'butt';
  for (let i = -3; i <= 3; i++) {
    const t = i / 3;
    ctx.beginPath();
    ctx.moveTo(cx + top * t, cy - h);
    ctx.lineTo(cx + r * t, cy + Math.cos(t * 1.5) * r * 0.36);
    ctx.stroke();
  }
  // Aros de mimbre.
  for (const k of [0.34, 0.66, 0.92]) {
    const rr = top + (r - top) * k;
    ctx.strokeStyle = k > 0.6 ? shade(wD, 0.1) : wD;
    ctx.beginPath();
    ctx.ellipse(cx, cy - h * (1 - k), rr, rr * 0.4, 0, 0.15, Math.PI - 0.15);
    ctx.stroke();
  }
}

/** Borriquete: dos patas en aspa y su barrote, para aserrar encima. */
function sawHorse(ctx, cx, cy, sc, wood, woodD) {
  ctx.strokeStyle = woodD; ctx.lineWidth = 2 * sc; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(cx - 6 * sc, cy); ctx.lineTo(cx + 4 * sc, cy - 13 * sc);
  ctx.moveTo(cx + 6 * sc, cy); ctx.lineTo(cx - 4 * sc, cy - 13 * sc);
  ctx.stroke();
  ctx.strokeStyle = wood; ctx.lineWidth = 2.4 * sc;
  ctx.beginPath();
  ctx.moveTo(cx - 5 * sc, cy - 11 * sc); ctx.lineTo(cx + 5 * sc, cy - 11 * sc);
  ctx.stroke();
}

/** Tronco tumbado, con su testa y sus anillos. */
function log(ctx, cx, cy, len, r, tone) {
  ctx.fillStyle = tone;
  ctx.fillRect(cx - len / 2, cy - r, len, r * 2);
  ctx.beginPath(); ctx.ellipse(cx + len / 2, cy, r * 0.55, r, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = shade(tone, -0.14);
  ctx.fillRect(cx - len / 2, cy + r * 0.3, len, r * 0.7);
  ctx.fillStyle = shade(tone, 0.24);
  ctx.beginPath(); ctx.ellipse(cx - len / 2, cy, r * 0.55, r, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = shade(tone, -0.3); ctx.lineWidth = 0.7;
  ctx.beginPath(); ctx.ellipse(cx - len / 2, cy, r * 0.26, r * 0.48, 0, 0, Math.PI * 2); ctx.stroke();
}
...
/** Mata de helechos: hojas puntiagudas, más viva que una mancha de hierba. */
function fern(ctx, cx, cy, s, tone) {
  ctx.lineCap = 'round';
  for (const [dx, dy, w, k] of [[-4.2, -3.6, 1.5, -0.2], [-2, -6, 1.7, -0.1],
    [0.4, -6.8, 1.8, 0], [2.6, -5.6, 1.7, 0.1], [4.4, -3, 1.5, 0.2]]) {
    ctx.strokeStyle = k < 0 ? shade(tone, -0.14) : tone;
    ctx.lineWidth = w * s;
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.quadraticCurveTo(cx + dx * 0.4 * s, cy + dy * 0.75 * s, cx + dx * s, cy + dy * s);
    ctx.stroke();
  }
  ctx.fillStyle = shade(tone, -0.24);
  ctx.beginPath(); ctx.ellipse(cx, cy, 2.6 * s, 1.2 * s, 0, 0, Math.PI * 2); ctx.fill();
}
...
function drawBuilding(ctx, type, colorIdx, x, y) {
  ...
  switch (type) {
    case 'house': {
      // Casa de aldea en ele: la crujía larga, con su chimenea de sillares y
      // el hastial grande de cara, y el ala corta cruzada, más baja y con los
      // paños de yeso claro. En el rincón que dejan las dos, el porche sobre
      // la puerta. Tejado de ripia y ventanas encendidas.
      const h = 30, glow = L.glow || '#ffc356';
      const band = [0.24, 0.46];
      // El ala va de otro color, como en las casas de entramado de verdad:
      // dos manos de yeso distintas sobre la misma madera.
      const [w2L, w2, w2D] = ramp(L.wall2 || L.wall);
      const M2 = { ...M, wall: w2, wallL: w2L, wallD: w2D };
      // Dos cajas que no se solapan, para que la de delante nunca pise a la
      // de detrás.
      const pA = iso(x, y, 0.12, 0.06);
      const fa = timberBlock(ctx, pA[0], pA[1], 1.06, 1.56, h, M, { windows: false, band });
      // Chimenea de sillares, adosada al muro largo.
      const [chL, chM, chD] = ramp(L.chimney || '#5c5c58');
      const ch = iso(x, y, 0, 0.6);
      isoPrism(ctx, ch[0], ch[1], 0.28, 0.28, 50, chL, chD, chM);
      stoneTexture(ctx, ch[0], ch[1], 0.28, 0.28, 50);
      const cap = iso(x, y, -0.03, 0.57);
      isoPrism(ctx, cap[0], cap[1] - 50, 0.34, 0.34, 2.4, chL, chD, chM);
      const pB = iso(x, y, 1.18, 0.2);
      const fb = timberBlock(ctx, pB[0], pB[1], 0.72, 0.92, h, M2, { windows: false, band });
      // Ventanas: dos grandes en la fachada larga y una en cada cara del ala.
      // La contraventana va de madera, no del color del jugador: aquí el azul
      // es cosa de los paños de abajo.
      for (let i = 0; i < 2; i++) {
        faceWindow(ctx, fa[0].a, fa[0].b, h, 0.3 + i * 0.4, 0.54, fa[0].wood, woodD, glow);
      }
      faceWindow(ctx, fb[0].a, fb[0].b, h, 0.68, 0.54, fb[0].wood, woodD, glow);
      faceWindow(ctx, fb[1].a, fb[1].b, h, 0.5, 0.54, fb[1].wood, woodD, glow);
      // Puerta, en el trozo de fachada que deja libre el ala.
      faceDoor(ctx, fa[1].a, fa[1].b, h, 0.14, 0.72, woodL, wood, L.door);
      // --- Tejado en ele ---
      // La crujía larga manda: su cumbrera queda por encima de la del ala, y
      // las dos se encuentran en una lima.
      const gA = roofOn(ctx, pA[0], pA[1], 1.06, 1.56, h, 17, roofD, roofM, wallL, mat,
        { axis: 'v', gableBack: roofD });
      const rB = iso(x, y, 0.82, 0.2);
      const gB = roofOn(ctx, rB[0], rB[1], 1.08, 0.92, h, 12, roofD, roofL, w2L, mat,
        { gableBack: roofD });
      gableTimbers(ctx, gA.br, gA.fr, gA.r1, wood, woodD);
      gableTimbers(ctx, gB.br, gB.fr, gB.r1, wood, woodD);
      // --- Porche sobre la puerta ---
      for (const v of [1.2, 1.62]) {
        const p = iso(x, y, 1.34, v);
        isoPrism(ctx, p[0], p[1], 0.13, 0.13, 18.5, woodL, woodD, wood);
      }
      const por = iso(x, y, 1.18, 1.16);
      const pr = shedRoof(ctx, por[0], por[1], 0.22, 0.56, 21, 18.5, 'u+', roofM, roofD, mat);
      // Una línea de sombra bajo el alero del porche, o se confunde con el
      // faldón grande que le cae justo encima.
      ctx.strokeStyle = 'rgba(24,18,10,.42)'; ctx.lineWidth = 1.4; ctx.lineCap = 'butt';
      ctx.beginPath();
      ctx.moveTo(pr.eA[0], pr.eA[1] + 3); ctx.lineTo(pr.eB[0], pr.eB[1] + 3); ctx.stroke();
      // --- La vida de la casa, alrededor ---
      // La leña se apila contra el muro, no en medio del corral.
      const yard = iso(x, y, 0.58, 1.82), keg = iso(x, y, 0.02, 1.96);
      logPile(ctx, yard[0], yard[1], wood, woodL, woodD, 0.8);
      barrel(ctx, keg[0], keg[1], 0.9, wood, woodD, stoneD);
      const box = iso(x, y, 1.94, 0.78);
      crate(ctx, box[0], box[1], 0.85, wood, woodD);
      const pit = iso(x, y, 1.88, 1.7);
      well(ctx, pit[0], pit[1], 6.5, M);
      for (const [u, v, sc] of [[0.06, 1.24, 1], [1.16, 1.8, 0.85], [1.98, 1.28, 0.95],
        [1.6, 0.06, 0.8], [0.9, 1.86, 0.7]]) {
        const p = iso(x, y, u, v);
        fern(ctx, p[0], p[1], sc, green);
      }
      // Piedras sueltas por el suelo.
      for (const [u, v, r] of [[1.5, 1.9, 1.7], [1.3, 1.98, 1.2], [1.02, 1.72, 1.1],
        [1.72, 1.96, 1.4], [0.34, 1.56, 1.2], [1.66, 1.42, 1]]) {
        const p = iso(x, y, u, v);
        ctx.fillStyle = stone;
        ctx.beginPath(); ctx.ellipse(p[0], p[1] - r * 0.4, r, r * 0.72, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = stoneD;
        ctx.beginPath(); ctx.ellipse(p[0], p[1], r, r * 0.42, 0, 0, Math.PI); ctx.fill();
      }
      break;
    }
    case 'towncenter': {
      ...
      const gB = roofOn(ctx, cross[0], cross[1], cw, rw + 0.14, eave, 26, roofD, roofL, wallL, 'tile',
        { axis: 'v', gableBack: roofD });
      gableTimbers(ctx, gA.br, gA.fr, gA.r1, wood, woodD, 'rich');
      gableTimbers(ctx, gB.br, gB.fr, gB.r1, wood, woodD, 'rich');
    ...
    case 'lumbercamp': {
      // No es un cobertizo: es un tajo de aserrar al raso. Manda la grúa de
      // pértiga --el poste sobre su cesto de mimbre lastrado, con el
      // contrapeso de piedras a un lado y el gancho al otro-- y alrededor se
      // reparten el banco de aserrar, la pila de troncos, la tablazón y el
      // carro. Los colores del jugador van en los anillos del poste y en el
      // fardo, que es donde se ven sin ensuciar la madera.
      const [soilL, soilM, soilD] = ramp(L.soil || '#a8926a');
      const [wkL, wkM, wkD] = ramp(L.wicker || '#b08a52');
      const bark = L.accent || '#8f6a3c';
      // Suelo pisado y lleno de virutas.
      const patch = (k) => [
      ...
      for (const [u, v, a] of [[1.2, 1.2, 0.4], [0.7, 1.5, 1.9], [1.55, 0.9, 0.8],
        [1.0, 0.55, 2.4], [1.45, 1.5, 1.2], [0.55, 0.95, 0.3]]) {
        const p = iso(x, y, u, v);
        ctx.save();
        ctx.translate(p[0], p[1]); ctx.rotate(a);
        ctx.fillStyle = soilL;
        ctx.fillRect(-3, -0.7, 6, 1.4);
        ctx.restore();
      }
      // --- Carro de arrastre y el fardo del jugador ---
      // Carro de troncos: un bastidor de varas tumbado y su rueda.
      const cart = iso(x, y, 0.22, 0.66);
      ctx.save();
      ctx.translate(cart[0], cart[1] - 4);
      ctx.rotate(0.46);
      ctx.fillStyle = wood;
      ctx.fillRect(-14, -6, 28, 2.2);
      ctx.fillRect(-14, 1.4, 28, 2.2);
      ctx.fillStyle = woodD;
      for (const dx of [-11, -4, 3, 10]) ctx.fillRect(dx, -6, 2, 9.6);
      ctx.fillStyle = woodL;
      ctx.fillRect(-14, -6, 28, 1);
      ctx.restore();
      cartWheel(ctx, cart[0] + 13, cart[1] + 3, 5.5, wood, woodD);
      // Fardo de tela con los colores del jugador, atado sobre un cajón.
      const bale = iso(x, y, 0.06, 1.16);
      crate(ctx, bale[0], bale[1], 0.75, wood, woodD);
      ctx.fillStyle = col.main;
      ctx.beginPath(); ctx.ellipse(bale[0] - 0.5, bale[1] - 7.4, 5.2, 3.4, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = col.light;
      ctx.beginPath(); ctx.ellipse(bale[0] - 2.4, bale[1] - 8.4, 1.9, 1.2, 0, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = col.dark;
      ctx.fillRect(bale[0] - 5.6, bale[1] - 7.8, 10, 1.2);
      // --- Grúa de pértiga ---
      const cr = iso(x, y, 0.92, 0.7);
      wickerBase(ctx, cr[0], cr[1], 13, 11, wkL, wkM, wkD);
      const topY = cr[1] - 11 - 40;
      // Poste, algo más delgado arriba que abajo.
      poly(ctx, [[cr[0] - 3.4, cr[1] - 10], [cr[0] + 3.4, cr[1] - 10],
        [cr[0] + 2.4, topY], [cr[0] - 2.4, topY]], wood);
      poly(ctx, [[cr[0] + 0.8, cr[1] - 10], [cr[0] + 3.4, cr[1] - 10],
        [cr[0] + 2.4, topY], [cr[0] + 0.6, topY]], woodD);
      // Anillos del color del jugador: uno al pie y otro bajo la pértiga.
      for (const [yy, rr] of [[cr[1] - 11, 4.6], [topY + 7, 3.6]]) {
        ctx.fillStyle = col.main;
        ctx.beginPath(); ctx.ellipse(cr[0], yy, rr, rr * 0.5, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = col.dark;
        ctx.beginPath(); ctx.ellipse(cr[0], yy, rr, rr * 0.5, 0, 0.2, Math.PI - 0.2); ctx.fill();
      }
      // Pértiga: el contrapeso cae del extremo corto y el gancho del largo.
      const bA = [cr[0] - 24, topY + 10], bB = [cr[0] + 30, topY - 9];
      ctx.strokeStyle = woodD; ctx.lineWidth = 4.4; ctx.lineCap = 'round';
      ctx.beginPath(); ctx.moveTo(bA[0], bA[1]); ctx.lineTo(bB[0], bB[1]); ctx.stroke();
      ctx.strokeStyle = woodL; ctx.lineWidth = 1.6;
      ctx.beginPath(); ctx.moveTo(bA[0], bA[1] - 1.4); ctx.lineTo(bB[0], bB[1] - 1.4); ctx.stroke();
      ctx.fillStyle = woodD;
      ctx.beginPath(); ctx.ellipse(cr[0], topY + 1, 4, 3.4, 0, 0, Math.PI * 2); ctx.fill();
      // Contrapeso: un cesto de piedras colgado de dos cuerdas.
      ctx.strokeStyle = '#cfc4a8'; ctx.lineWidth = 0.9; ctx.lineCap = 'butt';
      ctx.beginPath();
      ctx.moveTo(bA[0] - 5, bA[1] + 1); ctx.lineTo(bA[0] - 1, bA[1] + 13);
      ctx.moveTo(bA[0] + 6, bA[1] + 1); ctx.lineTo(bA[0] + 2, bA[1] + 13);
      ctx.stroke();
      const bw = [bA[0] + 0.5, bA[1] + 15];
      ctx.fillStyle = wkD;
      ctx.beginPath(); ctx.ellipse(bw[0], bw[1], 9, 4.6, 0, 0, Math.PI); ctx.fill();
      ctx.fillStyle = wkM;
      ctx.beginPath(); ctx.ellipse(bw[0], bw[1], 9, 3.6, 0, 0, Math.PI * 2); ctx.fill();
      for (const [dx, dy, r] of [[-4, -1, 3], [1, -2.4, 3.4], [5, -0.6, 2.6]]) {
        ctx.fillStyle = stone;
        ctx.beginPath(); ctx.ellipse(bw[0] + dx, bw[1] + dy, r, r * 0.78, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = stoneL;
        ctx.beginPath();
        ctx.ellipse(bw[0] + dx - r * 0.3, bw[1] + dy - r * 0.3, r * 0.4, r * 0.3, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      // Gancho: del extremo largo cuelga un tronco a media altura.
      ctx.strokeStyle = '#cfc4a8'; ctx.lineWidth = 0.9;
      ctx.beginPath();
      ctx.moveTo(bB[0], bB[1] + 1); ctx.lineTo(bB[0], bB[1] + 13); ctx.stroke();
      log(ctx, bB[0], bB[1] + 16, 16, 3.4, bark);
      // --- Banco de aserrar, con el tronco y la sierra ---
      const bench = iso(x, y, 0.6, 1.6);
      sawHorse(ctx, bench[0] - 13, bench[1] + 4, 1, wood, woodD);
      sawHorse(ctx, bench[0] + 14, bench[1] - 3, 1, wood, woodD);
      ctx.save();
      ctx.translate(bench[0], bench[1] - 10);
      ctx.rotate(-0.46);
      log(ctx, 0, 0, 34, 3.6, bark);
      ctx.restore();
      // La sierra, cruzada sobre el tronco y con su empuñadura.
      ctx.save();
      ctx.translate(bench[0] + 3, bench[1] - 12.5);
      ctx.rotate(-0.46);
      ctx.fillStyle = '#a9adb5';
      ctx.fillRect(-12, -1.2, 22, 1.4);
      ctx.strokeStyle = '#7d818a'; ctx.lineWidth = 0.6;
      for (let i = -11; i < 10; i += 1.8) {
        ctx.beginPath(); ctx.moveTo(i, 0.2); ctx.lineTo(i + 0.9, 1.4); ctx.stroke();
      }
      ctx.fillStyle = woodD;
      ctx.fillRect(10, -2, 3.4, 3.4);
      ctx.restore();
      // --- Pila de troncos entre dos caballetes ---
      const rack = iso(x, y, 1.62, 0.78);
      const stand = (dx) => {
        ctx.strokeStyle = woodD; ctx.lineWidth = 2; ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(rack[0] + dx - 5, rack[1] + 4); ctx.lineTo(rack[0] + dx + 4, rack[1] - 14);
        ctx.moveTo(rack[0] + dx + 5, rack[1] + 4); ctx.lineTo(rack[0] + dx - 4, rack[1] - 14);
        ctx.stroke();
      };
      stand(-14);
      for (const [dx, dy, t] of [[0, 0, 0], [0.5, -6.4, 1], [1, -12.4, 2]]) {
        log(ctx, rack[0] + dx, rack[1] - 3 + dy, 26, 3.4, t === 2 ? shade(bark, 0.14) : t === 1 ? bark : shade(bark, -0.08));
      }
      stand(13);
      // --- Tablazón aserrada, delante de todo ---
      const planks = iso(x, y, 1.32, 1.56);
      for (let i = 0; i < 3; i++) {
        const tone = i % 2 ? shade(L.wood, 0.16) : shade(L.wood, 0.28);
        ctx.fillStyle = tone;
        ctx.fillRect(planks[0] - 13 + i * 1.6, planks[1] - 2 - i * 3, 26, 3);
        ctx.strokeStyle = 'rgba(0,0,0,.22)'; ctx.lineWidth = 0.7;
        ctx.strokeRect(planks[0] - 13 + i * 1.6, planks[1] - 2 - i * 3, 26, 3);
      }
      for (const [u, v, sc] of [[0.12, 1.7, 0.9], [1.86, 1.1, 0.8], [1.05, 0.12, 0.75]]) {
        const p = iso(x, y, u, v);
        fern(ctx, p[0], p[1], sc, green);
      }
      break;
